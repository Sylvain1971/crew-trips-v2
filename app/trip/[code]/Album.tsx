...
import { toSignedUrlsBatch, extractPath, getStoredUrlForPath, deleteFiles } from '@/lib/storage'
...
export default function Album({ tripId, trip, membre, onTripUpdate }: { tripId: string, trip: Trip, membre: Membre, onTripUpdate: (t: Partial<Trip>) => void }) {
  ...
  // Phase 2 : bucket privé. On signe toutes les URLs d'images à l'affichage.
  // signedMap est : URL originale stockée en DB -> URL signée valide 1h.
  // Cache en mémoire pour éviter re-signing à chaque render.
  ...
  // Phase 2 : bucket privé. Signer les URLs dès que photos change.
  // Filtre les URLs non-signées et non-blob, puis batch-sign.
  useEffect(() => {
    const urls = photos
      .map(p => p.image_url)
    ...
    try {
      ...
      for (let i = 0; i < compressed.length; i++) {
        ...
        try {
          ...
          // Phase 2 : bucket privé. On stocke une URL "publique" (en fait un
          // wrapper contenant le path) qu'on signera à l'affichage.
  ...
  async function deleteSelected() {
    ...
    try {
      // Phase 2 : bucket privé. On utilise deleteFiles qui extrait le path
      // depuis n'importe quelle URL (publique ou signée).
