...
export default function Chat({ tripId, trip, membre }: { tripId: string, trip: Trip, membre: Membre }) {
  ...
  // Charger la page initiale (les plus recents)
  useEffect(() => {
    supabase
      .from('messages')
      .select('*')
    ...
    // Realtime : nouveaux messages + suppressions
    const ch = supabase
      .channel(`chat-${tripId}`)
  ...
  const loadMore = useCallback(async () => {
    ...
    if (data) {
      ...
      setHasMore(data.length === PAGE_SIZE)
      if (ordered.length > 0) oldestCreatedAt.current = ordered[0].created_at
      // Preserver la position de scroll lors de l'ajout en tete
  ...
  // Upload + insert : texte seul, photo seule, ou photo + legende
  async function send() {
    const t = txt.trim()
  ...
  async function deleteMessage(m: Message) {
    ...
    try {
      if (m.image_url) {
        // Extraire le path a partir de l'URL publique
        const match = m.image_url.match(/\/trip-photos\/(.+?)(\?|$)/)
        if (match) {
          const path = decodeURIComponent(match[1])
          // Best-effort : on ignore une erreur storage (la ligne DB est deja supprimee ce qui compte)
          await supabase.storage.from('trip-photos').remove([path]).catch(err => {
      ...
      // Rollback : remettre le message (position d'origine)
      setMsgs(prev => {
        const next = [...prev, m]
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', position: 'relative' }}>
          ...
          return (
            <div key={m.id} style={{ display: 'flex', gap: 8, alignItems: 'flex-end', flexDirection: mine ? 'row-reverse' : 'row' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', maxWidth: '74%', alignItems: mine ? 'flex-end' : 'flex-start', gap: 3 }}>
                ...
                {/* Barre d'actions : date + bouton Supprimer (seulement sur tes messages) */}
                <div style={{ display: 'flex', gap: 10, alignItems: 'center', paddingLeft: 4, paddingRight: 4 }}>
      ...
      {/* Barre d'envoi : positionnee au-dessus de la nav du bas (64px + safe-area) */}
      <div style={{ position: 'fixed', left: 0, right: 0, bottom: `calc(env(safe-area-inset-bottom,0px) + 64px)`, background: 'rgba(255,255,255,.97)', backdropFilter: 'blur(10px)', borderTop: '1px solid var(--border)', padding: '9px 12px 10px', display: 'flex', flexDirection: 'column', gap: 8, zIndex: 40 }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end' }}>
          {/* Bouton photo (visible seulement si canPostPhotos) */}
