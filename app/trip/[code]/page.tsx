...
function NavIcon({ tab }: { tab: Tab }) {
  if (tab === 'infos') return <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/></svg>
  if (tab === 'chat') return <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
  return <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
}
...
export default function TripPage({ params: paramsPromise }: { params: Promise<{ code: string }> }) {
  ...
  if (error || !trip) return (
    <div style={{minHeight:'100dvh',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:14,padding:24,textAlign:'center',background:'var(--forest)'}}>
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',minHeight:'100dvh',background:'var(--sand)'}}>
      {/* Header compact - seulement pour chat et membres */}
      {tab !== 'infos' && (
        <div style={{background:'var(--forest)',padding:'12px 16px 10px',display:'flex',alignItems:'center',gap:10,flexShrink:0}}>
          ...
          <div style={{flex:1,minWidth:0}}>
            <div style={{fontWeight:700,fontSize:15,color:'#fff',letterSpacing:'-.01em',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{trip.nom}</div>
          </div>
      ...
      <div style={{flex:1,overflow:tab==='chat'?'hidden':'auto',display:'flex',flexDirection:'column'}}>
        ...
        {tab === 'membres' && <Membres tripId={trip.id} tripCode={trip.code} membre={membre} />}
      </div>
