  ...
  return (
    <main style={{minHeight:'100dvh',display:'flex',flexDirection:'column',alignItems:'center',background:'var(--forest)',padding:'56px 20px 40px',position:'relative'}}>
      ...
      {/* Bouton + pour créer un nouveau trip — symétrique au ⚙️.
           44×44 = taille minimale recommandée par Apple HIG pour éviter les
           mistaps. bottom 32 + left 28 = marge confortable des bords courbés
           de l'iPhone + safe area. */}
      <button onClick={()=>router.push('/nouveau')} aria-label="Créer un nouveau trip"
        style={{position:'fixed',bottom:32,left:28,
      ...
      <a href="/admin" aria-label="Administration" style={{position:'fixed',bottom:32,right:28,
        background:'rgba(255,255,255,.1)',border:'1px solid rgba(255,255,255,.15)',
        borderRadius:'50%',width:44,height:44,display:'flex',alignItems:'center',justifyContent:'center',
